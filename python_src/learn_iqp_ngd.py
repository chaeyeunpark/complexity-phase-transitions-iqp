...
if __name__ == '__main__':
    ...
    state = np.load(data_path)[idx,:]
    target_probs = jnp.array(abs2(state))
    ...
    jtu.tree_map(lambda x: print(x.shape), params)

    fisher_and_grad = FisherAndGrad(N, model)

    params = jtu.tree_map(lambda x: x.astype(jnp.float64), params)
    ngd = SecondOrderMethod(learning_rate = learning_rate, beta1 = 0.9, beta2 = beta2)

    @partial(jax.jit, static_argnums=(0,))
    def step(fisher_and_grad, params, ngd, key, model_samples, target_samples):
        fisher, grad = fisher_and_grad.from_sample(params, model_samples, target_samples)
        ngd.update_momemtum(fisher, grad)
    ...
    for n_iter in range(TOTAL_ITER):
        ...
        params, ngd = step(fisher_and_grad, params, ngd, key_r, model_samples, target_samples)
    ...
    with Path('l1_dist.dat').open('w') as l1_dist_out, \
          Path('kl_div.dat').open('w') as kl_div_out:
        for n_iter in range(TOTAL_ITER):
            # Main training loop
            model_log_probs = fisher_and_grad.log_probs_func(params)
            model_sampler = ExactSampler(model_log_probs)
            ...
            if n_iter % RECORD_PER == 0:
                model_probs = jax.nn.softmax(model_log_probs)
                dist = l1_dist(target_probs, model_probs)
                print(f'{n_iter}\t{dist}', file = l1_dist_out)
                div = kl_div(model_probs, target_probs)
                print(f'{n_iter}\t{div}', file = kl_div_out)
        ...
        model_probs = jax.nn.softmax(model_log_probs)
        dist = l1_dist(target_probs, model_probs)
        print(f'{TOTAL_ITER}\t{dist}', file=l1_dist_out)
        div = kl_div(model_probs, target_probs)
        print(f'{TOTAL_ITER}\t{div}', file = kl_div_out)
    """
